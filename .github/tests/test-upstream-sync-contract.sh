...
test_version_match_repo_relative() { expected_version="$(node -p "require('$ROOT/package.json').version")" || return 1; output="$(cd "$ROOT" && EXPECTED_VERSION="$expected_version" bash "$VERSION_SCRIPT" 2>&1)" || return 1; assert_contains "$output" "package_version=$expected_version"; }
