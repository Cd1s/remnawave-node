...
if [ "${1:-}" = release ] && [ "${2:-}" = view ]; then
    if [ "${GH_BEHAVIOR:-}" = resolve ]; then printf '%s\n' '{"tagName":"v2.8.0","url":"https://github.com/remnawave/node/releases/tag/v2.8.0","publishedAt":"2026-08-01T00:00:00Z","isDraft":false,"isPrerelease":false}'; exit 0; fi
    case "${GH_BEHAVIOR:-}" in
