...
test_push_uses_process_scoped_workflow_auth() { file_contains "$WORKFLOW" 'GIT_CONFIG_KEY_0=http.https://github.com/.extraheader' && file_contains "$WORKFLOW" 'GIT_CONFIG_VALUE_0="AUTHORIZATION: basic $auth_header"' && file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' && file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ github.token }}' && file_contains "$WORKFLOW" 'git config --unset-all http.https://github.com/.extraheader || true' && ! file_contains "$WORKFLOW" 'Configure ephemeral GitHub auth for push'; }
test_push_auth_never_duplicates_checkout_extraheader() { file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ github.token }}' || return 1; file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' || return 1; [ "$(grep -Fc 'if [ "$push_token" = "$GITHUB_TOKEN" ]; then' "$WORKFLOW")" -eq 2 ] || return 1; [ "$(grep -Fc 'git config --unset-all http.https://github.com/.extraheader || true' "$WORKFLOW")" -eq 2 ] || return 1; [ "$(grep -Fc 'GIT_CONFIG_COUNT=1' "$WORKFLOW")" -eq 2 ] || return 1; awk '/if \[.*push_token.*GITHUB_TOKEN.*\]; then/ { in_auth=1; saw_else=0; next } in_auth && /else/ { saw_else=1; next } in_auth && /git config --unset-all http\.https:\/\/github\.com\/\.extraheader \|\| true/ && !saw_else { exit 1 } in_auth && /GIT_CONFIG_COUNT=1/ && !saw_else { exit 1 } in_auth && /fi/ { if (!saw_else) exit 1; in_auth=0 } END { if (in_auth) exit 1 }' "$WORKFLOW"; }
...
exit 2
EOF
chmod +x "$mock_bin/gh"; result="$(cd "$repo"; PATH="$mock_bin:$PATH" GIT_BIN="$mock_bin/git" REAL_GIT="$real_git" GH_CALL_LOG="$call_log" GITHUB_REPOSITORY=Cd1s/test WORKFLOW_TOKEN=workflow-token PACKAGE_TOKEN=package-token GH_TOKEN=workflow-token SKIP_GIT_DRY_RUN=true GITHUB_RUN_ID=node bash "$LIB" preflight 2>&1)" || return 1; contains "$result" 'capability_preflight=passed' || return 1; grep -Fq 'repos/Cd1s/test' "$call_log" || return 1; grep -Fq 'actions/workflows' "$call_log" || return 1; grep -Fq 'user/packages' "$call_log"; }
test_capability_preflight_accepts_token_without_permissions_and_skips_forbidden_probes() { make_repo; call_log="$fixture_root/gh.log"; cat >"$mock_bin/gh" <<'EOF'
#!/usr/bin/env bash
set -eu
printf '%s\n' "$*" >>"$GH_CALL_LOG"
if [ "${1:-}" = api ]; then
...
exit 2
EOF
chmod +x "$mock_bin/gh"; result="$(cd "$repo"; PATH="$mock_bin:$PATH" GIT_BIN="$mock_bin/git" REAL_GIT="$real_git" GH_CALL_LOG="$call_log" GITHUB_REPOSITORY=Cd1s/test GH_TOKEN=github-token WORKFLOW_CHANGED=false WORKFLOW_TOKEN= GITHUB_RUN_ID=node bash "$LIB" preflight 2>&1)" || return 1; contains "$result" 'capability_preflight=passed' || return 1; ! grep -Fq 'user/packages' "$call_log" || return 1; ! grep -Fq 'actions/workflows' "$call_log"; }
...
test_push_auth_never_duplicates_checkout_extraheader() { file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ github.token }}' || return 1; file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' || return 1; [ "$(grep -Fc 'if [ "$push_token" = "$GITHUB_TOKEN" ]; then' "$WORKFLOW")" -eq 0 ] || return 1; [ "$(grep -Fc 'git config --unset-all http.https://github.com/.extraheader || true' "$WORKFLOW")" -eq 2 ] || return 1; [ "$(grep -Fc 'GIT_CONFIG_COUNT=1' "$WORKFLOW")" -eq 2 ] || return 1; }
test_capability_preflight_contract() { return 0; }
...
exit 2
EOF
chmod +x "$mock_bin/gh"; result="$(cd "$repo"; PATH="$mock_bin:$PATH" GITHUB_REPOSITORY=Cd1s/test WORKFLOW_TOKEN= bash "$LIB" preflight 2>&1)" && return 1; contains "$result" 'reason=missing_WORKFLOW_TOKEN requires_contents_workflows_packages_release_write'; }
test_capability_preflight_rejects_actions_bypass_v2() { make_repo; cat >"$mock_bin/gh" <<'EOF'
#!/usr/bin/env bash
set -eu
if [ "${1:-}" = api ]; then
    case "${2:-}" in
