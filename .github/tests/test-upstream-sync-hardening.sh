...
test_push_uses_process_scoped_workflow_auth() { file_contains "$WORKFLOW" 'GIT_CONFIG_KEY_0=http.https://github.com/.extraheader' && file_contains "$WORKFLOW" 'GIT_CONFIG_VALUE_0="AUTHORIZATION: basic $auth_header"' && file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && file_contains "$WORKFLOW" 'WORKFLOW_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && ! file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ github.token }}' && ! file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' && ! file_contains "$WORKFLOW" 'push_token=' && file_contains "$WORKFLOW" 'git config --unset-all http.https://github.com/.extraheader || true' && ! file_contains "$WORKFLOW" 'Configure ephemeral GitHub auth for push'; }
...
exit 2
EOF
chmod +x "$mock_bin/gh"; result="$(cd "$repo"; PATH="$mock_bin:$PATH" GIT_BIN="$mock_bin/git" REAL_GIT="$real_git" GH_CALL_LOG="$call_log" GITHUB_REPOSITORY=Cd1s/test GH_TOKEN=workflow-token WORKFLOW_TOKEN=workflow-token SKIP_GIT_DRY_RUN=true GITHUB_RUN_ID=node bash "$LIB" preflight 2>&1)" && return 1; contains "$result" 'reason=workflows_write_denied' || return 1; grep -Fq 'actions/workflows' "$call_log" && ! grep -Fq 'user/packages' "$call_log"; }
test_workflow_diff_without_token_fails_closed() { make_repo; result="$(cd "$repo"; PATH="$mock_bin:$PATH" GITHUB_REPOSITORY=Cd1s/test WORKFLOW_CHANGED=true WORKFLOW_TOKEN= bash "$LIB" preflight 2>&1)" && return 1; contains "$result" 'reason=missing_WORKFLOW_TOKEN requires_contents_workflows_packages_release_write'; }
test_push_auth_never_duplicates_checkout_extraheader() { ! file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ github.token }}' || return 1; ! file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' || return 1; ! file_contains "$WORKFLOW" 'push_token=' || return 1; [ "$(grep -Fc 'git config --unset-all http.https://github.com/.extraheader || true' "$WORKFLOW")" -eq 2 ] || return 1; [ "$(grep -Fc 'GIT_CONFIG_COUNT=1' "$WORKFLOW")" -eq 2 ] || return 1; }
