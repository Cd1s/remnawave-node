...
test_checkout_and_readonly_resolver_use_workflow_token() { file_contains "$WORKFLOW" 'token: ${{ secrets.WORKFLOW_TOKEN }}' && file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && ! file_contains "$WORKFLOW" 'token: ${{ secrets.WORKFLOW_TOKEN || secrets.GITHUB_TOKEN }}'; }
run_case() { if "$1"; then pass "$1"; else fail "$1"; fi; }; run_case test_resolver_stable; run_case test_workflow_and_dual_core_contract; run_case test_upstream_tag_fetch_is_namespaced; run_case test_checkout_and_readonly_resolver_use_workflow_token; [ "$failures" -eq 0 ] || exit 1; printf 'all upstream hardening tests passed\n'
