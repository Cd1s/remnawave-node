...
test_upstream_tag_fetch_is_namespaced() { file_contains "$WORKFLOW" 'refs/tags/${{ steps.release.outputs.tag }}:refs/tags/upstream-release-${{ steps.release.outputs.tag }}'; }
run_case() { if "$1"; then pass "$1"; else fail "$1"; fi; }; run_case test_resolver_stable; run_case test_workflow_and_dual_core_contract; run_case test_upstream_tag_fetch_is_namespaced; [ "$failures" -eq 0 ] || exit 1; printf 'all upstream hardening tests passed\n'
