...
test_checkout_and_readonly_resolver_use_builtin_token() { file_contains "$WORKFLOW" 'token: ${{ secrets.GITHUB_TOKEN }}' && ! file_contains "$WORKFLOW" 'token: ${{ secrets.WORKFLOW_TOKEN }}' && file_contains "$WORKFLOW" 'GH_TOKEN: ${{ github.token }}' && file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN || secrets.GITHUB_TOKEN }}' && file_contains "$WORKFLOW" 'WORKFLOW_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}'; }
test_push_uses_process_scoped_workflow_auth() { file_contains "$WORKFLOW" 'GIT_CONFIG_KEY_0=http.https://github.com/.extraheader' && file_contains "$WORKFLOW" 'GIT_CONFIG_VALUE_0="AUTHORIZATION: basic $auth_header"' && file_contains "$WORKFLOW" 'GH_TOKEN: ${{ secrets.WORKFLOW_TOKEN || secrets.GITHUB_TOKEN }}' && file_contains "$WORKFLOW" 'WORKFLOW_TOKEN: ${{ secrets.WORKFLOW_TOKEN }}' && file_contains "$WORKFLOW" 'GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}' && file_contains "$WORKFLOW" 'WORKFLOW_CHANGED: ${{ steps.sync.outputs.workflow_changed }}' && file_contains "$WORKFLOW" 'push_token="$GITHUB_TOKEN"' && file_contains "$WORKFLOW" 'push_token="$WORKFLOW_TOKEN"' && ! file_contains "$WORKFLOW" 'PACKAGE_TOKEN' && file_contains "$WORKFLOW" 'git config --unset-all http.https://github.com/.extraheader || true' && ! file_contains "$WORKFLOW" 'Configure ephemeral GitHub auth for push'; }
